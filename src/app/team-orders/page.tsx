...
export default function TeamOrdersPage() {
  ...
  return (
    <div className="font-body w-full">
      ...
      <StaggerGroup className="max-w-[1240px] mx-auto px-6 pt-8.5 pb-4.5 grid grid-cols-[repeat(auto-fit,minmax(230px,1fr))] gap-3.5">
        {STEPS.map((s) => (
          <StaggerItem key={s.num} className="bg-card border border-white/8 px-5 py-5.5 flex flex-col gap-2">
      ...
      <section className="max-w-[1240px] mx-auto px-6 pt-4.5 pb-15 grid grid-cols-[repeat(auto-fit,minmax(300px,1fr))] gap-8.5 items-start">
        <Reveal className="flex flex-col gap-4.5">
            ...
            <div className="flex gap-2 flex-wrap">
              {PIECE_LABELS.map((label) => (
                <motion.button
                  ...
                  className={chipClass(pieces.includes(label))}
                >
                  {label}
                </motion.button>
              ))}
          ...
          <div className="flex flex-col gap-2.5">
            ...
            <div className="flex gap-2 flex-wrap">
              {PRINT_LABELS.map((label) => (
                <motion.button
                  key={label}
                  onClick={() => togglePrint(label)}
                  whileTap={{ scale: 0.95 }}
                  transition={{ duration: 0.15 }}
                  className={chipClass(prints.includes(label))}
                >
                  {label}
                </motion.button>
              ))}
          ...
          <div className="flex flex-col gap-2.5">
            <div className="flex items-center justify-between gap-3.5">
              ...
              <p className="m-0 font-display font-extrabold text-[15px] text-white overflow-hidden">
          ...
          <AnimatePresence mode="wait" initial={false}>
            {submitted && ok ? (
              <motion.div
                ...
                className="bg-card border border-lime/40 px-5.5 py-5 flex flex-col gap-1.5"
              >
              ...
              >
                <motion.button
                  onClick={submit}
                  whileHover={{ backgroundColor: "#D8FF4A" }}
                  whileTap={{ scale: 0.97 }}
                  transition={{ duration: 0.18 }}
                  className="self-start bg-lime text-ground border-none font-display font-extrabold text-[13px] tracking-[0.1em] uppercase px-6 py-4 cursor-pointer inline-flex items-center gap-2.5"
                >
        ...
        <div className="flex flex-col gap-3.5">
          <Reveal className="bg-ground-alt border border-white/9 p-5.5 flex flex-col gap-3.25">
            <p className="m-0 font-display font-extrabold text-xs tracking-[0.16em] uppercase text-white">
              Indicative estimate
            </p>
            <div className="flex items-center justify-between gap-3.5">
            ...
            <div className="flex items-end justify-between gap-3.5 pt-3.25 border-t border-white/12">
              ...
              <p className="m-0 font-display font-black text-2xl text-white overflow-hidden">
                <AnimatePresence mode="popLayout" initial={false}>
                  <motion.span
                    key={pieceAvg ? money(total) : "—"}
                    initial={{ opacity: 0, y: 8 }}
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: -8 }}
                    transition={{ duration: 0.18, ease: EASE }}
          ...
          <StaggerGroup className="bg-ground-alt border border-white/9 p-5.5 flex flex-col gap-2.75">
            <p className="m-0 font-display font-extrabold text-xs tracking-[0.16em] uppercase text-white">
              What&apos;s included
            </p>
            {INCLUDED.map((i) => (
              <StaggerItem key={i} className="grid grid-cols-[10px_minmax(0,1fr)] gap-3 items-start">
          ...
          <Reveal delay={0.1} className="bg-lime text-ground px-5.5 py-6 flex flex-col gap-3">
