...
// The prototype's "Tees" / "Crewnecks" filters split on a subcategory the
// catalog doesn't track, and every Team piece here is a tee, so "Tees"
// mirrors "All" honestly (same treatment as the Men's page).
const FILTERS = ["All", "Tees"] as const;
type Filter = (typeof FILTERS)[number];
type Sort = "featured" | "low" | "high";

const teamProducts = byCategory("Team");
...
export default function TeamPage() {
  ...
  const shown = useMemo(() => {
    void filter; // both filters currently resolve to the full Team catalog
    ...
    <div className="font-body w-full">
      ...
      <section className="relative overflow-hidden bg-ground-alt border-b border-hairline/90">
        <div
          ...
          style={{ backgroundImage: "url(/assets/team-back-banner.jpg)", backgroundPosition: "center 42%" }}
        />
        ...
        <div className="relative z-10 max-w-[1240px] mx-auto px-6 pt-13 pb-12 flex items-end justify-between gap-7.5 flex-wrap">
          <div className="flex flex-col gap-3.5 max-w-[44ch]">
            ...
            <p className="m-0 text-[15px] leading-[1.7] text-[#C4C4C4]">
              Stronger together. The same fabrics, carrying your club crest, names and numbers.
            </p>
          </div>
          <MarkerNote lines={["Stronger", "together", "always"]} rotate={-9} swashWidth={116} />
        </div>
      </section>
      ...
      <section className="grid grid-cols-[repeat(auto-fit,minmax(320px,1fr))] bg-ground border-t border-hairline/90">
        <div className="relative min-h-[300px] overflow-hidden">
          <Image
            ...
            alt="HUDA Sports team orders"
            fill
            className="object-cover [object-position:center_42%] [filter:contrast(1.05)_brightness(0.98)]"
          />
        ...
        <div className="bg-lime text-ground px-10 py-12 flex flex-col justify-center gap-4.5">
          ...
          <p className="m-0 text-[15px] font-medium leading-[1.65] max-w-[40ch]">
            From ten pieces up: names, numbers and club logos, quoted in two working days.
          </p>
